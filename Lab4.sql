...
CREATE TABLE IF NOT EXISTS ecommerce_site.supplier(
...
(select `order`.*,customer.cus_name,cus_gender from `order` inner join customer where `order`.cus_id=customer.cus_id
having `order`.ord_amount>=3000) as t1 group by t1.cus_id) as t2 group by t2.cus_gender;
